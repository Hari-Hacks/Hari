"""Main exploration loop for the robot mapping project."""
...
class ExplorerController:
    ...
    def step_once(self) -> dict[str, object]:
        ...
        if frontier is None or plan is None:
            if current_cell in frontiers:
                ...
                step_info["message"] = "Current cell still borders unknown space, so the robot rotates to inspect another global direction."
                return step_info
            self.finished = True
        ...
        step_info["motor_commands"] = self.to_motor_commands(movement_commands)
        step_info["motor_text"] = " ".join(step_info["motor_commands"]) or "HOLD"
        step_info["message"] = "Executed the first low-turn segment of the heading-aware plan toward the nearest frontier."
        return step_info
    ...
    def run(self, max_iterations: int = 64) -> None:
        ...
        for _ in range(max_iterations):
            ...
            print(f"Pose: ({step_info['pose'][0]:.2f}, {step_info['pose'][1]:.2f})")
            print(f"Grid cell: {step_info['grid_cell']}")
            print(f"Heading: {step_info['heading']}")
            print(f"IR values used for mapping: {step_info['ir_values']}")
            print(f"Cell updates: {step_info['cell_updates']}")
            print(f"Visited cells: {step_info['visited_cells']}")
            print(f"Scan commands: {step_info['scan_commands'] or ['HOLD']}")
            print(f"Selected frontier: {step_info['selected_frontier']}")
            print(f"Planned path: {step_info['planned_path']}")
            print(f"Planned actions: {step_info['planned_actions'] or ['HOLD']}")
            print(f"Movement commands: {step_info['movement_commands'] or ['HOLD']}")
        ...
        print()
        print(f"Executed {len(self.command_history)} total commands.")
