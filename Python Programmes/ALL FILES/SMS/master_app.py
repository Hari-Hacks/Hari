                ...
                if (data.latest_image && data.latest_image !== lastImage) {
                    ...
                    if (data.inference_meta && data.inference_meta.boxes && data.inference_meta.image === data.latest_image) {
                        ...
                        data.inference_meta.boxes.forEach(box => {
                            ...
                            const confPercent = (box.conf * 100).toFixed(1);
                            boxesHtml += `
                                <div class="bounding-box" style="left: ${left}%; top: ${top}%; width: ${width}%; height: ${height}%;">
                                    <div style="position: absolute; top: 0; left: 0; background: var(--danger); color: white; padding: 2px 6px; font-size: 12px; font-weight: bold; border-radius: 0 0 3px 0; white-space: nowrap; z-index: 20;">
                                        Person ${confPercent}%
                                    </div>
                                </div>`;
                        });
...
def alert():
    ...
    # Send notification at most once every 10 seconds to avoid spam
    if current_time - last_notification_time > 10:
        try:
            # Using ntfy.sh for free, instant push notifications
            # Download the 'ntfy' app and subscribe to 'vision_iot_alerts_admin'
            topic = "vision_iot_alerts_admin" 
            requests.post(f"https://ntfy.sh/{topic}", 
                          data="ALERT: Person detected in restricted zone!".encode('utf-8'),
                          headers={"Title": "Security Alert", "Tags": "rotating_light,warning"})
            last_notification_time = current_time
            print(f"Sent mobile notification to ntfy.sh/{topic}")
    ...
    while True:
        ...
        for file in files:
            if file.endswith(".jpg") and file not in processed_files:
                ...
                output_path = os.path.join(INFERENCE_FOLDER, output_name)
...
if __name__ == "__main__":
    # Start the YOLO inference loop in a background thread
    inference_thread = threading.Thread(target=run_inference_loop, daemon=True)
    inference_thread.start()

    # Automatically open the web browser to the admin page
